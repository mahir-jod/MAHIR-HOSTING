...
def init_db():
    ...
    if not db.execute('SELECT * FROM admin_settings WHERE id=1').fetchone():
        db.execute('INSERT INTO admin_settings (id, username, password) VALUES (1, "MAHIRJOD", "MAHIRJOD")')
...
# Add these routes to your Flask app (app.py)
    ...
    @app.route('/server/get-startup/<folder>')
    def get_startup(folder):
        db = get_db()
        srv = db.execute('SELECT startup FROM servers WHERE folder=?', (folder,)).fetchone()
        db.close()
        return jsonify({'startup': srv['startup'] if srv else 'main.py'})

    @app.route('/server/uptime/<folder>')
    def get_uptime_api(folder):
        online = False
        uptime_str = "Offline"
        if folder in running_procs and running_procs[folder].poll() is None:
            online = True
            if folder in start_times:
                uptime_str = get_precise_uptime(start_times[folder])
        else:
            db = get_db()
            srv = db.execute('SELECT pid FROM servers WHERE folder=?', (folder,)).fetchone()
            db.close()
            if srv and srv['pid'] and psutil.pid_exists(srv['pid']):
                online = True
                # Try to get start time from system
                try:
                    proc = psutil.Process(srv['pid'])
                    uptime_str = get_precise_uptime(proc.create_time())
                except:
                    uptime_str = "Online"
        return jsonify({'online': online, 'uptime': uptime_str})

    @app.route('/server/command/<folder>', methods=['POST'])
    def send_command(folder):
        # This sends commands to the running process (optional feature)
        # For simplicity, we'll just write to console.log
